    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center justify-between border-b border-border bg-card/80 px-4 py-3 backdrop-blur md:hidden">
          ...
          <Link
            to="/profile"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-accent text-secondary"
          >
            <UserIcon size={18} />
          </Link>
        </header>
            ...
            {NAV.map(({ to, label, icon: Icon }) => {
              ...
              return (
                <Link
                  ...
                  to={to}
                  className={cn(
                    "flex min-w-[56px] flex-1 flex-col items-center gap-0.5 rounded-lg px-2 py-2 text-[11px] font-medium transition-colors",
                    active
                      ? "text-primary"
